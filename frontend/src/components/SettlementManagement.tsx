...
import { User, Group, Settlement, groupAPI, settlementAPI } from '../api';
...
const SettlementManagement: React.FC<SettlementManagementProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    try {
      const response = await groupAPI.getSettlements(selectedGroup.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedGroup || formData.payer_id === 0 || formData.payee_id === 0 || formData.amount <= 0) {
      return;
    }
    ...
    setLoading(true);
    try {
      await groupAPI.addSettlement(selectedGroup.id, {
    ...
    <div className="space-y-6">
      ...
      {!selectedGroup ? (
        <div className="bg-white shadow-md rounded-lg p-6">
          ...
          <p className="text-gray-600 mb-4">Choose a group to record settlements:</p>
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">
                Payment History for {selectedGroup.name} ({settlements.length})
              </h3>
            </div>
